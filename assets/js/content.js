...
function renderHome(data) {
  contentRoot.innerHTML = `<section class="hero section"><div class="container hero-grid"><div><p class="eyebrow">${escapeHtml(data.profile.affiliation)}</p><h1>${escapeHtml(data.profile.name)}</h1><p class="lead">${escapeHtml(data.profile.tagline)}</p><div class="button-row"><a class="button" href="research.html">Explore my research</a><a class="button button-secondary" href="cv.html">View CV</a></div><div class="profile-links">${data.links.slice(0, 4).map((link) => externalLink(link.url, link.label)).join("")}</div></div><div class="profile-placeholder" aria-label="Profile photo placeholder"><span>Profile photo</span></div></div></section><section class="section section-muted"><div class="container"><p class="eyebrow">Current work</p><h2>Research at the intersection of autonomy and field robotics</h2><div class="card-grid section-compact">${data.researchThemes.map((theme) => `<article class="card"><h3>${escapeHtml(theme.title)}</h3><p>${escapeHtml(theme.description)}</p></article>`).join("")}</div></div></section><section class="section"><div class="container"><div class="section-heading"><div><p class="eyebrow">Selected work</p><h2>Recent projects</h2></div><a href="projects.html">View all projects</a></div><div class="card-grid">${data.projects.slice(0, 3).map(projectCard).join("")}</div></div></section>`;
}

function renderAbout(data) {
  contentRoot.innerHTML = `<section class="section"><div class="container content-width"><p class="eyebrow">About</p><h1>${escapeHtml(data.profile.name)}</h1>${data.profile.bio.map((text) => `<p class="bio-copy">${escapeHtml(text)}</p>`).join("")}<dl class="facts"><div><dt>Position</dt><dd>${escapeHtml(data.profile.title)}</dd></div><div><dt>Affiliation</dt><dd>${escapeHtml(data.profile.affiliation)}</dd></div><div><dt>Supervisor</dt><dd>${escapeHtml(data.profile.supervisor)}</dd></div></dl><h2>Experience</h2>${timeline(data.experience, "experience")}<h2 class="content-section-title">Education</h2>${timeline(data.education, "education")}</div></section>`;
}
